// Ce script utilise l'API Admin de Firebase pour manipuler directement la base de données
// Pour l'exécuter, vous devez d'abord installer firebase-admin:
...
// Vous devez télécharger ce fichier depuis la console Firebase
...
try {
  ...
  admin.initializeApp({
    ...
    databaseURL: "https://whikser-a5580.firebaseio.com",
  });
} catch (error) {
  console.error("Erreur lors de l'initialisation de Firebase Admin:", error);
  console.error(
    "\nVous devez télécharger un fichier serviceAccountKey.json depuis la console Firebase:"
  );
  console.error("1. Allez sur https://console.firebase.google.com/");
  console.error("2. Sélectionnez votre projet");
  console.error("3. Allez dans Project settings > Service accounts");
  console.error("4. Cliquez sur 'Generate new private key'");
  console.error(
    "5. Placez le fichier téléchargé à la racine du projet et renommez-le 'serviceAccountKey.json'"
  );
  process.exit(1);
}
...
const deleteAllPets = async () => {
  try {
    ...
    const deletePromises = petsSnapshot.docs.map((doc) => doc.ref.delete());
